...
def combination(goal1, goal2, goal3, agent1_loc, agent2_loc, agent3_loc, folder):
    ...
    if folder == 'legibility':
        ...
        tau9 = np.asarray([x*2 for x in goal1]+goal3+agent3_loc)
        tau10 = np.asarray(goal3+[x*2 for x in goal1]+goal3)
        ...
        # only two agents have goals, one agent does two tasks
        tau4 = np.asarray([x*2 for x in goal1] + goal3 + agent3_loc)
        tau5 = np.asarray(goal3 + agent2_loc + [x*2 for x in goal1])
        tau6 = np.asarray(agent1_loc + [x*2 for x in goal1] + goal2)
        # one agent has three goals
        tau7 = np.asarray(agent1_loc + agent2_loc + [x*3 for x in goal1])
        tau8 = np.asarray([x*3 for x in goal1] + agent2_loc + agent3_loc)
        tau9 = np.asarray(agent1_loc + [x*3 for x in goal1] + agent3_loc)
        G = [tau1, tau2, tau3, tau4, tau5,
        tau6, tau7, tau8, tau9]
